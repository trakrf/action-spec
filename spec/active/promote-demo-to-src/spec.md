...
## Origin
This specification follows from D2A-25 (archiving overengineered implementation). After archiving the complex Lambda/API Gateway code, we now promote the working POC from `/demo` to `/src` as the official main implementation.

**Linear Issue**: D2A-26
**Branch**: `refactor/promote-demo-to-src`

## Outcome
The `/demo` directory will be moved to `/src`, making the working Flask/Jinja POC the official main implementation. All references, imports, and configurations will be updated to reflect this promotion.

## User Story
As a **portfolio reviewer or future contributor**
I want **the main implementation to live in `/src` not `/demo`**
...
**Desired State**:
- Working implementation at `/src/` signals "main application codebase"
...
- Clear separation: `/overengineered/` (archived), `/src/` (app), `/infra/` (IaC), `/demo/` (removed)
- Clean import paths: `from src.backend` not `from demo.backend`
...
**Application Code** (moves to `/src/`):
- [ ] Move `/demo/backend/` → `/src/backend/`
- [ ] Move `/demo/Dockerfile` → `/src/Dockerfile`
- [ ] Move `/demo/README.md` → `/src/README.md` (app-specific docs with app)
- [ ] Move `/demo/requirements.txt` → `/src/backend/requirements.txt` (if not already there)
...
- [ ] Update Python imports in all `.py` files:
  - `from demo.backend` → `from src.backend`
...
- [ ] Update `docker-compose.yml` (now at root):
  - Volume mounts from `./demo/backend` → `./src/backend`
  - Build context from `./demo` → `./src`
  - Working directories
...
- [ ] Update main `README.md`:
  - References to `/demo` → `/src`
  - Remove "POC" or "demo" framing
  - Update Quick Start instructions
- [ ] Update `/demo/README.md` → `/src/README.md`:
...
### Quality Gates
- [ ] Flask app runs: `cd src/backend && flask run`
...
2. **Move application code**: Backend, Dockerfile → `/src/`
...
5. **Move documentation**: SPEC.md → `/docs/`, README.md → `/src/`
...
- You run `docker compose up` and `just <command>` from root, not from `/src`
...
**Success Metrics**:
- [ ] Flask app starts successfully from `/src/backend/`
- [ ] Docker compose up works from `/src/`
- [ ] All Python imports work (no ImportError exceptions)
- [ ] GitHub Actions CI passes
- [ ] No references to `/demo/` in active code (only git history)
- [ ] Main README clearly describes `/src` as main implementation
...
# Test imports
cd src/backend && python -c "from backend.app import app; print('✓ Imports work')"
...
**From Linear D2A-26**:
- Goal: "Promote working POC from `/demo` to `/src` as the main implementation"
