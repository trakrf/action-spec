...
def test_api_reachable():
    ...
    try:
        ...
        response = requests.get(
            f"{API_ENDPOINT}/api/parse", headers={"x-api-key": API_KEY}, timeout=10
        )
...
def test_endpoint_returns_200(endpoint, method):
    ...
    if method == "GET":
        response = requests.get(
            f"{API_ENDPOINT}{endpoint}", headers=headers, timeout=10
        )
    else:
...
def test_endpoint_returns_valid_json(endpoint):
    ...
    headers = {"x-api-key": API_KEY}
    response = requests.post(
        f"{API_ENDPOINT}{endpoint}", headers=headers, json={}, timeout=10
    )
